...
\subsection{Contexto y Objetivo Principal}

En el campo del Aprendizaje Automático (\textit{Machine Learning}), el rendimiento de un
modelo no depende únicamente de la calidad de los datos de entrenamiento, sino de forma
crítica de la configuración de sus \textbf{hiperparámetros}. A diferencia de los parámetros
internos que el modelo aprende durante el ajuste, los hiperparámetros deben ser definidos
a priori y gobiernan la estructura misma del algoritmo y su capacidad de generalización.

El objetivo principal de esta práctica es \textbf{desarrollar y aplicar un Algoritmo
Genético Adaptativo para la optimización de hiperparámetros del modelo
\texttt{RandomForestClassifier}} de la librería scikit-learn, con el fin de maximizar la
precisión (\textit{accuracy}) de clasificación sobre el dataset ``Wine Quality''.

\subsection{Cumplimiento de Requisitos del Proyecto}

Para garantizar el rigor técnico de la investigación, el sistema ha sido diseñado
respetando estrictamente los siguientes requisitos técnicos obligatorios:
...
\subsection{Preparación del Dataset: Clasificación Binaria}
...
% =============================================================
\section{Configuración del Modelo y Espacio de Búsqueda}
% =============================================================
...
% =============================================================
\section{Función de Aptitud (\textit{Fitness})}
% =============================================================
...
\subsection{Robustez de la Evaluación: Estratificación y K-Fold}
...
\begin{itemize}[itemsep=0.2em]
  ...
  \item \textbf{Con semilla fija (\texttt{random\_state=42}):} La evaluación se vuelve
        ...
        uso de \textbf{memoización} (caché), ahorrando horas de cómputo al no reevaluar
...
% =============================================================
\section{Algoritmos Implementados: Definición y Justificación}
% =============================================================
...
\begin{enumerate}[itemsep=0.2em]
  ...
  \item \textbf{Eficiencia:} Con la memoización, no reevalúa configuraciones repetidas,
        siendo más eficiente que los dos métodos anteriores ante el coste computacional
        real de entrenar un Random Forest.
\end{enumerate}

% =============================================================
\section{Arquitectura Detallada del Algoritmo Genético}
% =============================================================

\subsection{El Descubrimiento del Cuello de Botella Genético}
...
Para prevenirlo, la población inicial de $N_p = 30$ individuos se construye con un
...
\subsection{Elitismo y Memoización: Garantías de Calidad}
...
dicha generación. Esto garantiza que los 30 individuos de la población sean siempre
exploradores únicos, eliminando el desperdicio de cómputo en clones.

\textbf{Memoización por caché de fitness.} Entrenar un Random Forest es
computacionalmente costoso. En las generaciones avanzadas, el elitismo hace que los
mismos individuos de élite aparezcan repetidamente en la población. Sin memoización,
...
\begin{lstlisting}[language=Python, caption={Mecanismo de memoización del fitness}]
...
\subsection{Control Adaptativo de \texorpdfstring{$P_c$}{Pc} y \texorpdfstring{$P_m$}{Pm}: El Motor Inteligente}
...
\begin{enumerate}[itemsep=0.3em]
  ...
  \item \textbf{Algoritmo Genético:} 5 ejecuciones independientes con $N_p = 30$
        individuos únicos y 50 generaciones cada una.
\end{enumerate}
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    \midrule
    Random Search (\textit{RS})           & $0.7667$ & $0.7667$ & $\pm 0.00$ \\
    Grid Search (\textit{GS})             & $0.8123$ & $0.8123$ & (Determinista) \\
    \textbf{Algoritmo Genético (GA)}      & $\mathbf{0.8154}$ & $\mathbf{0.8154}$ & $\pm 0.00$ \\
    \bottomrule
  \end{tabular}}
\end{table}
\renewcommand{\arraystretch}{1}
...
\begin{figure}[H]
  \centering
  \IfFileExists{img/comparison_boxplot.png}{\includegraphics[width=0.85\textwidth]{img/comparison_boxplot.png}}{\fbox{Imagen no generada aún}}
...
\begin{figure}[H]
  \centering
  \IfFileExists{img/ga_convergence.png}{\includegraphics[width=0.85\textwidth]{img/ga_convergence.png}}{\fbox{Imagen no generada aún}}
...
\begin{figure}[H]
  \centering
  \IfFileExists{img/ga_adaptive_params.png}{\includegraphics[width=0.85\textwidth]{img/ga_adaptive_params.png}}{\fbox{Imagen no generada aún}}
...
\begin{figure}[H]
  \centering
  \IfFileExists{img/runs_bar_chart.png}{\includegraphics[width=0.85\textwidth]{img/runs_bar_chart.png}}{\fbox{Imagen no generada aún}}
...
\begin{figure}[H]
  \centering
  \IfFileExists{img/rs_score_histogram.png}{\includegraphics[width=0.85\textwidth]{img/rs_score_histogram.png}}{\fbox{Imagen no generada aún}}
...
\begin{figure}[H]
  \centering
  \IfFileExists{img/gs_heatmap.png}{\includegraphics[width=0.85\textwidth]{img/gs_heatmap.png}}{\fbox{Imagen no generada aún}}
...
\begin{figure}[H]
  \centering
  \IfFileExists{img/eval_cost_comparison.png}{\includegraphics[width=0.70\textwidth]{img/eval_cost_comparison.png}}{\fbox{Imagen no generada aún}}
...
continua (sin rejilla), el RS obtiene un score considerablemente inferior
(0.7667 vs.\ 0.8123). Con solo 100 evaluaciones aleatorias, la cobertura de
un espacio de 10 dimensiones es estadísticamente baja; y, sobre todo, el RS carece
de cualquier mecanismo de aprendizaje que le permita concentrar los esfuerzos en las
regiones prometedoras. Esto queda evidenciado en la Figura~\ref{fig:histogram}:
la mayoría de los 500 scores del RS se agrupan por debajo de 0.80, demostrando
que el muestreo ciego rara vez alcanza la región de alta calidad del espacio.

\textbf{Grid Search vs.~Algoritmo Genético.} El GA supera al GS (0.8154 vs.\ 0.8123)
...
únicos de modelo por método. Gracias a la memoización, el GA realiza entre 200 y 400
evaluaciones únicas por ejecución ---comparable al RS--- pero cada una de ellas está
guiada informativamente por la selección evolutiva, lo que explica la mejora cualitativa.
...
% =============================================================
\section{Conclusiones y Crítica del Experimento}
% =============================================================
...
\begin{enumerate}[itemsep=0.3em]
  \item \textbf{Superación de los baselines.} El Algoritmo Genético Adaptativo ha logrado
        la puntuación más alta (\textbf{0.8154}), superando tanto la búsqueda aleatoria
        como la rejilla fija. Esto valida el uso de técnicas bio-inspiradas para problemas
        donde no existe información del gradiente.

  \item \textbf{El fenómeno de la llanura de rendimiento.} Una observación crucial es que las
        diferencias entre RS, GS y GA son relativamente pequeñas (en el rango de 1--4\%
        de mejora). Esto sugiere que el dataset Wine Quality con un Random Forest presenta un
        paisaje de búsqueda con una amplia región de ``buenos resultados''. En este
        escenario, es normal que un \textit{Random Search} obtenga resultados competitivos
        por simple probabilidad, y que sea difícil para el GA marcar una distancia abismal.

  \item \textbf{Eficiencia frente a la Dimensionalidad.} El verdadero valor del GA se
        manifiesta al considerar la complejidad del espacio. Con un cromosoma de 10 genes:
        \begin{itemize}
          \item Un \textbf{Grid Search} que quisiera probar solo 3 valores por cada gen
                necesitaría el entrenamiento de $3^{10} = 59.049$ modelos, algo
                computacionalmente prohibitivo.
          \item El \textbf{GA} ha navegado este espacio de forma continua y eficiente,
                encontrando mejores soluciones con solo una fracción ínfima de ese esfuerzo (aprox. 300--400 evaluaciones únicas).
        \end{itemize}

  \item \textbf{Importancia crítica del termómetro.} La lección técnica más valiosa ha sido
        que la calidad del algoritmo metaheurístico es irrelevante si la función de
        evaluación (\textit{fitness}) es ruidosa. La transición a \texttt{StratifiedKFold}
        con semilla fija fue lo que permitió que la evolución fuera realmente guiada por el
        mérito de los parámetros y no por el azar de las particiones.
\end{enumerate}
